...
static BandPreset s_selectedBand = FREQBAND_M1;
static uint32_t s_offset = 0; //display transcoder frequency offset

// Stepping
#define MAX_STEP_SIZE 100000000
#define MIN_STEP_SIZE 1
static uint32_t s_stepSize = 1000;

#define FREQUENCY_MIN   0 //   1.8 MHz
#define FREQUENCY_MAX 999999999 // 500 MHz
...
void FrequencyManager_Initialize(void)
{
	...
	s_offset = s_bandsData[FREQBAND_OFFSET].Setpoint;
...
void FrequencyManager_SetSelectedBand(BandPreset newBand)
{
	...
	if (s_selectedBand >= FREQBAND_OFFSET)
		s_stepSize = 100;
...
void FrequencyManager_SetCurrentFrequency(uint32_t newFrequency)
{
	...
	// Set the startup frequency (F0)
		...
		} else if (s_selectedBand == FREQBAND_OFFSET) {
			s_offset = (double) newFrequency;
...
// Output control
	...
	if (frequency != lastFrequency) {
		RS_HFIQ_Command_Freq(frequency);
		RS_HFIQ_Command_ClockDrive(0, 2);
...
{
	uint32_t newFreq = (FrequencyManager_GetCurrentFrequency()/s_stepSize + 1)*s_stepSize;
	FrequencyManager_SetCurrentFrequency(newFreq);
	FrequencyManager_Check_FilterBand(newFreq);

...
uint32_t I2C_WriteRegister16(uint8_t DeviceAddr, uint8_t RegisterAddr,
 ...
 *         fails.
 */
uint16_t I2C_ReadSlave16(uint8_t Address, uint8_t Register) {
	...
	// removing this causes bus to hang ??
	I2C_AcknowledgeConfig(I2C1, ENABLE);
...
//	while (!I2C_CheckEvent(I2C1,I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED)) {
	...
	/*!< Read the byte received from the Codec*/
	result = (uint16_t)I2C_ReceiveData(I2C1)<<8 ; // Receive high byte
	...
	/*!< Read the byte received from the Codec  and append*/
	result |= I2C_ReceiveData(I2C1 );
	...
	while (I2C1 ->CR1 & I2C_CR1_STOP ) {
		if ((Timeout--) == 0)
			return 10; //Codec_TIMEOUT_UserCallback();
	}

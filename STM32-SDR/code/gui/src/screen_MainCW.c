...
static _Bool FreqStatusUpdateHandler(GL_PageControls_TypeDef* pThis, _Bool forceRedisplay)
{
	...
	int val = (int) FrequencyManager_GetCurrentFrequency() + FrequencyManager_DisplayOffset();
	int i = 12, j=12;
	for(; i>=0 && j>=0 && val ; --i){
		if (i%3==0){
			buf[j] = ',';
...
void ScreenMainCW_Create(void)
{
	...
	//Frequency Label
		...
		AddPageControlObj(72, 145, pFreqLabel, s_pThisScreen);

		pTxFreqLabel = Widget_NewLabel("", LCD_COLOR_GREEN, LCD_COLOR_BLACK, 0, GL_FONTOPTION_16x24,FreqStatusUpdateHandler);
		AddPageControlObj(24, 115, pTxFreqLabel, s_pThisScreen); // IsTransparent
